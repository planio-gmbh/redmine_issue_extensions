...
1.  When the issue is edited, the person in charge is set to the issue, and the state of the issue is changed to new status if the state of the issue is old status, except when the issue bulk edit.
...
6. Goto Issue Extensions setting tab and select the status for old and new.
